    ...
    switch (action.type) {
        ...
        case AgendaItemParticipantActions.UpdateAgendaItemParticipants:
            ...
            return state.filter(x => !action.payload.participants
                .some(p => p.agendaItemId.id === x.agendaItemId.id &&
        ...
        case AgendaItemParticipantActions.SyncParticipantAgendaPresent:
            // Синхронизация отметки присутствия в уже загруженных вопросах в "Ожидает рассмотрения"
            const newState = state.filter(x => action.payload.participant.agendaItem.meeting.id === x.agendaItem.meeting.id &&
                action.payload.participant.agendaItem.issue.id !== x.agendaItem.issue.id &&
                action.payload.participant.person.id === x.person.id &&
                action.payload.agendaItems.some(ai => ai.issue.id === x.agendaItem.issue.id &&
                ai.meeting.id === x.agendaItem.meeting.id && ai.status === AgendaItemStatus.WaitingForConsideration)
            );

            newState.forEach(x => x.presents = action.payload.participant.presents);

            return state.filter(x => !newState
